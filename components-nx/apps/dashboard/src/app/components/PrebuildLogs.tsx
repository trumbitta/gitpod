...
export default function PrebuildLogs(props: PrebuildLogsProps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const info = await getGitpodService().server.getWorkspace(
          props.workspaceId
        );
        ...
        console.error(err);
        setError(err as SetStateAction<Error | undefined>);
      }
    })();
  ...
  useEffect(() => {
    ...
    if (workspaceInstance?.status.conditions.headlessTaskFailed) {
      setError(
        new Error(workspaceInstance.status.conditions.headlessTaskFailed)
      );
    }
...
export function watchHeadlessLogs(
  ...
  const startWatchingLogs = async () => {
    ...
    const retryBackoff = async (reason: string, err?: Error) => {
      ...
      delayInSeconds = Math.min(
        delayInSeconds * backoffFactor,
        maxBackoffSeconds
      );
    ...
    try {
      const logSources = await getGitpodService().server.getHeadlessLog(
        instanceId
      );
      ...
      await retryBackoff('error while listening to stream', err as Error);
    }
  };
  startWatchingLogs().catch(console.error);
