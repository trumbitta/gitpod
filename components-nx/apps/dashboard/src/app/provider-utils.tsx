...
function iconForAuthProvider(type: string) {
  switch (type) {
    case 'GitHub':
      return (
        <img
          className="fill-current dark:filter-invert w-5 h-5 ml-3 mr-3 my-auto"
          src={github}
        />
      );
    case 'GitLab':
      return (
        <img
          className="fill-current filter-grayscale w-5 h-5 ml-3 mr-3 my-auto"
          src={gitlab}
        />
      );
    case 'Bitbucket':
...
async function openAuthorizeWindow(params: OpenAuthorizeWindowParams) {
  ...
  const returnTo = gitpodHostUrl
    .with({ pathname: 'complete-auth', search: search })
    .toString();
...
const getSafeURLRedirect = (source?: string) => {
  const returnToURL: string | null = new URLSearchParams(
  ...
  if (returnToURL) {
    ...
    if (
      returnToURL
        .toLowerCase()
        .startsWith(
          `${window.location.protocol}//${window.location.host}/api/oauth/`.toLowerCase()
        )
    ) {
      return returnToURL;
    }
