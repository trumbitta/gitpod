...
> {
  ...
  async createWorkspace(
    ...
    const timeout = setTimeout(
      () => this.setState({ stillParsing: false }),
      3000
    );
    ...
    try {
      ...
      clearTimeout(timeout);
      console.error(error);
      this.setState({ error, stillParsing: false } as {
        error: StartWorkspaceError | undefined;
        stillParsing: boolean;
      });
    }
  }
  ...
  render() {
          ...
          statusMessage = (
            <div className="text-center">
              <p className="text-base mt-2">
                Are you trying to open a Git repository from a self-hosted
                instance?{' '}
                <a className="text-blue" href="https://gitpod.io/integrations">
                  Add integration
                </a>
              </p>
            </div>
    ...
    if (result?.createdWorkspaceId) {
      ...
      statusMessage = (
        <Modal visible={true} closeable={false} onClose={() => null}>
          ...
          <div className="border-t border-b border-gray-200 dark:border-gray-800 mt-4 -mx-6 px-6 py-2">
            <p className="mt-1 mb-2 text-base">
              You already have running workspaces with the same context. You can
              open an existing one or open a new workspace.
            </p>
            {result?.existingWorkspaces?.map((w) => (
              ...
              >
                <div className="w-full">
                  ...
                  <p className="truncate" title={w.workspace.contextURL}>
          ...
          <div className="flex justify-end mt-6">
            <button
              onClick={() => this.createWorkspace(CreateWorkspaceMode.Default)}
            >
              New Workspace
            </button>
          </div>
      ...
      return (
        <RunningPrebuildView
          ...
          onIgnorePrebuild={() =>
            this.createWorkspace(CreateWorkspaceMode.ForceNew)
          }
          onPrebuildSucceeded={() =>
            this.createWorkspace(CreateWorkspaceMode.UsePrebuild)
          }
        />
      );
    }
  ...
  return (
    <Modal visible={true} closeable={false} onClose={() => null}>
      ...
      <div className="flex justify-end mt-6">
        ...
        <a
          href={gitpodHostUrl.with({ pathname: 'plans' }).toString()}
          className="ml-2"
        >
...
function LimitReachedParallelWorkspacesModal() {
  return (
    <LimitReachedModal>
      <p className="mt-1 mb-2 text-base">
        You have reached the limit of parallel running workspaces for your
        account. Please, upgrade or stop one of the running workspaces.
      </p>
    </LimitReachedModal>
  );
}
...
function LimitReachedOutOfHours() {
  return (
    <LimitReachedModal>
      <p className="mt-1 mb-2 text-base">
        You have reached the limit of monthly workspace hours for your account.
        Please upgrade to get more hours for your workspaces.
      </p>
    </LimitReachedModal>
  );
}
...
function RepositoryNotFoundView(p: { error: StartWorkspaceError }) {
  ...
  useEffect(() => {
    (async () => {
      const { host, owner, repoName, userIsOwner, userScopes, lastUpdate } =
        p.error.data;
      ...
      const authProvider = (
        await getGitpodService().server.getAuthProviders()
      ).find((p) => p.host === host);
      if (!authProvider) {
        return;
      }
      ...
      const missingScope =
        authProvider.host === 'github.com' ? 'repo' : 'read_repository';
        ...
        setStatusMessage(
          <div className="mt-2 flex flex-col space-y-8">
            <p className="text-base text-gray-400 w-96">
              The repository '{`${repoFullName}`}' may be private. Please
              authorize Gitpod to access to private repositories.
            </p>
      ...
      if (!updatedRecently) {
        setStatusMessage(
          <div className="mt-2 flex flex-col space-y-8">
            <p className="text-base text-gray-400 w-96">
              Permission to access private repositories has been granted. If you
              are a member of '{owner}', please try to request access for
              Gitpod.
            </p>
            <a className="mx-auto" href={authorizeURL}>
              <button className="secondary">Request Access for Gitpod</button>
            </a>
          </div>
        );
        return;
      ...
      setStatusMessage(
        <div className="mt-2 flex flex-col space-y-8">
          <p className="text-base text-gray-400 w-96">
            Your access token was updated recently. Please try again if the
            repository exists and Gitpod was approved for '{owner}'.
          </p>
          <a className="mx-auto" href={authorizeURL}>
            <button className="secondary">Try Again</button>
          </a>
        </div>
      );
    })();
...
function RunningPrebuildView(props: RunningPrebuildViewProps) {
  ...
  let prebuildDoneTriggered = false;
  ...
  useEffect(() => {
    const checkIsPrebuildDone = async (): Promise<boolean> => {
      ...
      const done = await getGitpodService().server.isPrebuildDone(
        props.runningPrebuild.prebuildID
      );
      if (done) {
