...
export function WorkspaceEntry({ desc, model, isAdmin, stopWorkspace }: Props) {
  ...
  const state: WorkspaceInstancePhase =
    desc.latestInstance?.status?.phase || 'stopped';
  const currentBranch =
    desc.latestInstance?.status.repo?.branch ||
    Workspace.getBranchName(desc.workspace) ||
    '<unknown>';
  ...
  const updateWorkspaceDescription = async () => {
    ...
    try {
      ...
      await getGitpodService().server.setWorkspaceDescription(
        ws.id,
  ...
  return (
    <Item className="whitespace-nowrap py-6 px-6">
      ...
      <ItemField className="w-2/12 flex">
        <Tooltip
          content={`Created ${moment(desc.workspace.creationTime).fromNow()}`}
        >
        ...
        <div className="border-t border-b border-gray-200 dark:border-gray-800 -mx-6 px-6 py-4 space-y-2">
          ...
          <div className="mt-1">
            <p className="text-gray-500">
              Change the description to make it easier to go back to a
              workspace.
            </p>
            <p className="text-gray-500">
