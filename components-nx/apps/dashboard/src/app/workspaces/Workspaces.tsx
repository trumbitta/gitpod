...
export default function () {
  ...
  const match = useRouteMatch<{ team: string; resource: string }>(
    '/(t/)?:team/:resource'
  );
  const projectSlug =
    match?.params?.resource !== 'workspaces'
  ...
  const [isTemplateModelOpen, setIsTemplateModelOpen] =
  ...
  const fetchTeamsProjects = async () => {
    const projectsPerTeam = await Promise.all(
      (teams || []).map((t) => getGitpodService().server.getTeamProjects(t.id))
    );
    const allTeamsProjects = (projectsPerTeam as any).flat(1);
    ...
    (async () => {
      ...
      if (projectSlug) {
        project = projects.find((p) =>
          p.slug ? p.slug === projectSlug : p.name === projectSlug
        );
        if (project) {
          setProjects([project]);
        }
      ...
      if (project) {
        ...
        workspaceModel = new WorkspaceModel(
          ...
          setInactiveWorkspaces,
          getGitpodService()
            .server.getTeamProjects(team?.id)
        ...
        // Don't await
        const teamsProjectIdsPromise = fetchTeamsProjects().then((tp) =>
          tp.map((p: any) => p.id)
        );
        setTeamsWorkspaceModel(
          new WorkspaceModel(
            setTeamsActiveWorkspaces,
            () => null,
            teamsProjectIdsPromise,
            false
          )
        );
      }
  ...
  return (
    <>
      <Header
        title="Workspaces"
        subtitle="Manage recent and stopped workspaces."
      />

      {workspaceModel?.initialized &&
        (activeWorkspaces.length > 0 ||
        inactiveWorkspaces.length > 0 ||
        workspaceModel.searchTerm ? (
          <>
            <div className="app-container py-2 flex">
              <div className="flex">
                ...
                <input
                  ...
                  onChange={(v) => {
                    if (workspaceModel)
                      workspaceModel.setSearch(v.target.value);
                  }}
                />
              </div>
              ...
              <div className="py-3 pl-3">
                <DropDown
                  ...
                  activeEntry={
                    workspaceModel ? workspaceModel?.limit + '' : undefined
                  }
                  entries={[
                    {
                      title: '50',
              ...
              <button onClick={showStartWSModal} className="ml-2">
                New Workspace
              </button>
            </div>
            <ItemsList className="app-container">
              ...
              {teamsWorkspaceModel?.initialized && (
                <ActiveTeamWorkspaces
                  teams={teams}
                  teamProjects={teamsProjects}
                  teamWorkspaces={teamsActiveWorkspaces}
                />
              )}
              ...
              {inactiveWorkspaces.length > 0 && (
                <div className="p-3 text-gray-400 bg-gray-50 dark:bg-gray-800 rounded-xl text-sm text-center">
                  Unpinned workspaces that have been inactive for more than 14
              ...
              <div className="flex flex-col items-center justify-center h-96 w-96 mx-auto">
                {!!team && projects.length === 0 ? (
                  ...
                  <>
                    ...
                    <div className="text-center pb-6 text-gray-500">
                      Prefix any Git repository URL with {window.location.host}
                      /# or create a new workspace for a recently used project.{' '}
                      <a
                        className="gp-link"
      ...
      <StartWorkspaceModal
        ...
        recent={
          workspaceModel && activeWorkspaces ? getRecentSuggestions() : []
        }
      />
    </>
  );
...
function ActiveTeamWorkspaces(props: {
  teams?: Team[];
  teamProjects: Project[];
  teamWorkspaces: WorkspaceInfo[];
}) {
  if (!props.teams || props.teamWorkspaces.length === 0) {
    return <></>;
  }
  return (
    <div className="p-3 text-gray-400 bg-gray-50 dark:bg-gray-800 rounded-xl text-sm flex items-center justify-center space-x-1">
      <div className="mr-2 rounded-full w-3 h-3 bg-green-500" />
      <span>
        There are currently more active workspaces in the following teams:
      </span>
      <span>
        {props.teams
          .map((t) => {
            const projects = props.teamProjects.filter(
              (p) => p.teamId === t.id
            );
            const count = props.teamWorkspaces.filter((w) =>
              projects.some((p) => p.id === w.workspace.projectId)
            ).length;
            if (count < 1) {
              return undefined;
            }
            return (
              <Link className="gp-link" to={`/t/${t.slug}/workspaces`}>
                {t.name}
              </Link>
            );
          })
