...
import { countries, CountryCode } from 'countries-list';
import {
  AccountStatement,
  Subscription,
...
import {
  PlanCoupon,
  GithubUpgradeURL,
} from '@gitpod/gitpod-protocol/lib/payment-protocol';
import {
  Plans,
  Plan,
  Currency,
  PlanType,
} from '@gitpod/gitpod-protocol/lib/plans';
...
export default function () {
  ...
  const [gitHubUpgradeUrls, setGitHubUpgradeUrls] =
    useState<GithubUpgradeURL[]>();

  const [teamClaimModal, setTeamClaimModal] = useState<
    TeamClaimModal | undefined
  >(undefined);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      server.getClientRegion().then((v) => () => {
        setCurrency(
          countries[v as CountryCode]?.currency === 'EUR' ? 'EUR' : 'USD'
        );
      }),
  ...
  const activeSubscriptions = (accountStatement?.subscriptions || []).filter(
    (s) => Subscription.isActive(s, new Date().toISOString())
  );
  const freeSubscription =
    activeSubscriptions.find(
      (s) => s.planId === Plans.FREE_OPEN_SOURCE.chargebeeId
    ) || // Prefer Pro Open Source plan
  ...
  const paidSubscription = activeSubscriptions.find((s) =>
    UserPaidSubscription.is(s)
  );
  const paidPlan = paidSubscription && Plans.getById(paidSubscription.planId);

  const assignedTeamSubscriptions = activeSubscriptions.filter((s) =>
    AssignedTeamSubscription.is(s)
  );
  ...
  const assignedTs =
    assignedProfessionalTs || assignedUnleashedTs || assignedStudentUnleashedTs;

  const claimedTeamSubscriptionId = new URL(
      ...
      .then((freeSlot) => {
        if (!freeSlot) {
          setTeamClaimModal({
            mode: 'error',
            errorText:
              'This invitation is no longer valid. Please contact the team owner.',
          });
        } else {
          setTeamClaimModal({
            mode: 'confirmation',
  ...
  if (!!pendingUpgradePlan) {
    if (
      !!accountStatement &&
      paidPlan?.chargebeeId === pendingUpgradePlan.chargebeeId
    ) {
  ...
  const currentPlan =
    pendingUpgradePlan ||
    paidPlan ||
    Plans.getById(assignedTs?.planId) ||
    freePlan;
  ...
  const scheduledDowngradePlanId = !!paidSubscription?.paymentData
    ?.downgradeDate
  ...
  if (!!pendingDowngradePlan) {
    if (
      !!accountStatement &&
      (paidPlan || freePlan).chargebeeId === pendingDowngradePlan.chargebeeId
    ) {
  ...
  const [pendingDowngradeCancellation, setPendingDowngradeCancellation] =
    useState<Pending | undefined>(
  ...
  if (!!pendingDowngradeCancellation) {
    if (!!accountStatement && !scheduledDowngradePlanId) {
      ...
      pendingDowngradeCancellation.pendingSince + 1000 * 60 * 5 <
      Date.now()
    ) {
  ...
  const pendingChargebeeCallback =
    !!pendingUpgradePlan ||
    !!pendingDowngradePlan ||
    !!pendingDowngradeCancellation;
  ...
  const [isConfirmCancelDowngrade, setIsConfirmCancelDowngrade] =
  ...
  const confirmUpgrade = (to: Plan) => {
    ...
    if ((paidSubscription?.paymentReference || '').startsWith('github:')) {
      const url = (gitHubUpgradeUrls || []).find(
        (u) => u.planID == to.githubId
      );
      if (url) {
          ...
          ChargebeeClient.getOrCreate()
            .then((chargebeeClient) => {
              chargebeeClient.checkout(
                (paymentServer) =>
                  paymentServer.checkout(confirmUpgradeToPlan.chargebeeId),
                {
                  success: resolve,
                  error: reject,
                }
  ...
  const doDowngrade = async (event: React.MouseEvent) => {
    ...
    try {
      ...
      if (Plans.isFreePlan(confirmDowngradeToPlan.chargebeeId)) {
        await getGitpodService().server.subscriptionCancel(
          paidSubscription.uid
        );
      } else {
  ...
  const doCancelDowngrade = async (event: React.MouseEvent) => {
    ...
    try {
      await getGitpodService().server.subscriptionCancelDowngrade(
        paidSubscription.uid
      );
  ...
  if (currentPlan.chargebeeId === freePlan.chargebeeId) {
    ...
    let bottomLabel;
    if (
      scheduledDowngradePlanId === targetPlan.chargebeeId &&
      !pendingDowngradeCancellation
    ) {
      bottomLabel = (
      ...
      bottomLabel = (
        <p className="text-green-600 animate-pulse">Downgrade scheduled</p>
      );
    }
    let onDowngrade;
  ...
  const isUnleashedTsAssigned =
    !!assignedStudentUnleashedTs || !!assignedUnleashedTs;
  if (currentPlan.chargebeeId === studentUnleashedPlan.chargebeeId) {
    ...
    const targetPlan = applyCoupons(
      isStudent ? studentUnleashedPlan : unleashedPlan,
      availableCoupons
    );
    let onUpgrade;
    switch (Plans.subscriptionChange(currentPlan.type, targetPlan.type)) {
  ...
  return (
    <div>
      ...
      >
          ...
          <div className="w-full text-center">
            ...
            {!assignedTs && (
              <p className="text-base w-96 m-auto">
                Upgrade your plan to get more hours and more parallel
                workspaces.
              </p>
            )}
            <Tooltip
              content={`Current billing cycle: ${guessCurrentBillingCycle(
                currentPlan,
                accountStatement
              )
            ...
            typeof currentPlan.hoursPerMonth === 'number' ? (
              <progress
                value={
                  currentPlan.hoursPerMonth - accountStatement.remainingHours
                }
        ...
        <div className="mt-4 flex justify-center space-x-3 2xl:space-x-7">
          {planCards}
        </div>
        <InfoBox className="w-2/3 mt-14 mx-auto">
          If you are interested in purchasing a plan for a team, purchase a Team
          plan with one centralized billing.{' '}
        ...
        {!!confirmUpgradeToPlan && (
          <Modal
            visible={true}
            onClose={() => setConfirmUpgradeToPlan(undefined)}
          >
        ...
        {!!confirmDowngradeToPlan && (
          <Modal
            visible={true}
            onClose={() => setConfirmDowngradeToPlan(undefined)}
          >
            ...
            <div className="border-t border-b border-gray-200 dark:border-gray-800 mt-4 -mx-6 px-6 py-2">
              ...
              <InfoBox className="mb-4">
                {!Plans.isFreePlan(confirmDowngradeToPlan.chargebeeId) ? (
                  <span>
                    Your account will downgrade to {confirmDowngradeToPlan.name}{' '}
                    on the next billing cycle.
                  </span>
                ) : (
                  <span>
                    Your account will downgrade to {confirmDowngradeToPlan.name}
                    . The remaining hours in your current plan will be available
                    to use until the next billing cycle.
                  </span>
                )}
              </InfoBox>
            </div>
        ...
        {isConfirmCancelDowngrade && (
          <Modal
            visible={true}
            onClose={() => setIsConfirmCancelDowngrade(false)}
          >
            <h3>Cancel downgrade and stay with {currentPlan.name}</h3>
            <div className="border-t border-b border-gray-200 dark:border-gray-800 mt-4 -mx-6 px-6 py-2">
              <p className="mt-1 mb-4 text-base">
                You are about to cancel the scheduled downgrade and stay with{' '}
                {currentPlan.name}.
              </p>
              <InfoBox className="mb-4">
            ...
            <div className="flex justify-end mt-6">
              <button
                className="bg-red-600 border-red-800"
                onClick={doCancelDowngrade}
              >
                Cancel Downgrade
              </button>
            </div>
            ...
            <div className="flex justify-end mt-6">
              {teamClaimModal.mode === 'confirmation' && (
                <>
                  <button
                    className="secondary"
                    onClick={() => setTeamClaimModal(undefined)}
                  >
                    Cancel
                  </button>
                  <button
                    ...
                    onClick={async () => {
                      try {
                        ...
                        const statement =
                          await getGitpodService().server.getAccountStatement(
                            {}
                          );
                        setAccountStatement(statement);
                      } catch (error) {
                        setTeamClaimModal({
                          mode: 'error',
                          errorText: `Error: ${(error as any).message}`,
                        });
                      }
                    }}
                  >
...
function applyCoupons(
  plan: Plan,
  coupons: PlanCoupon[] | undefined
): PlanWithOriginalPrice {
  let coupon = (coupons || []).find(
    (c) => c.chargebeePlanID == plan.chargebeeId
  );
  if (!coupon) {
    return plan;
  }
...
function guessCurrentBillingCycle(
  currentPlan: Plan,
