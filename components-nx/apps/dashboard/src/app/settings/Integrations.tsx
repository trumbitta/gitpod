...
function GitProviders() {
  ...
  const [editModal, setEditModal] = useState<
    | {
        ...
        prevScopes: Set<string>;
        nextScopes: Set<string>;
      }
    | undefined
  >(undefined);
  const [selectAccountModal, setSelectAccountModal] = useState<
    SelectAccountPayload | undefined
  >(undefined);
  ...
  const updateCurrentScopes = async () => {
    if (user) {
      ...
      for (let provider of connectedProviders) {
        ...
        const token = await getGitpodService().server.getToken({
          host: provider.host,
        });
  ...
  const gitProviderMenu = (provider: AuthProviderInfo) => {
    ...
    if (connected) {
      ...
      const connectedWithSecondProvider = authProviders.some(
        (p) =>
          p.authProviderId !== provider.authProviderId &&
          isConnected(p.authProviderId)
      );
  ...
  const disconnect = async (ap: AuthProviderInfo) => {
    ...
    const returnTo = gitpodHostUrl
      .with({ pathname: 'complete-auth', search: 'message=success' })
      .toString();
  ...
  const startEditPermissions = async (provider: AuthProviderInfo) => {
    ...
    if (token) {
      setEditModal({
        provider,
        prevScopes: new Set(token.scopes),
        nextScopes: new Set(token.scopes),
      });
    }
  };
  ...
  const updatePermissions = async () => {
    ...
    try {
      await doAuthorize(
        editModal.provider.host,
        Array.from(editModal.nextScopes)
      );
    } catch (error) {
      console.log(error);
    }
  ...
  return (
    <div>
      {selectAccountModal && (
        <SelectAccountModal
          {...selectAccountModal}
          close={() => setSelectAccountModal(undefined)}
        />
      )}
            ...
            {(editModal.provider.scopes || []).map((scope) => (
              <div key={`scope-${scope}`}>
                <CheckBox
                  ...
                  disabled={editModal.provider.requirements?.default.includes(
                    scope
                  )}
          ...
          authProviders.map((ap) => (
            <Item key={'ap-' + ap.authProviderId} className="h-16">
              ...
              <ItemField className="hidden xl:w-5/12 xl:flex xl:flex-col">
                ...
                <span className="text-sm my-auto text-gray-400">
                  Permissions
                </span>
              </ItemField>
...
export function GitIntegrationModal(
    ...
    let entry =
      mode === 'new'
        ...
        : ({
            ...
            clientId,
            clientSecret:
              clientSecret === 'redacted' ? undefined : clientSecret,
          } as AuthProviderEntry.UpdateEntry);

    setBusy(true);
    setErrorMessage(undefined);
    try {
      const newProvider = await getGitpodService().server.updateOwnAuthProvider(
        { entry }
      );
  ...
  const validate = () => {
    ...
    if (clientId.trim().length === 0) {
      errors.push(
        `${type === 'GitLab' ? 'Application ID' : 'Client ID'} is missing.`
      );
    }
    if (clientSecret.trim().length === 0) {
      errors.push(
        `${type === 'GitLab' ? 'Secret' : 'Client Secret'} is missing.`
      );
    }
    if (errors.length === 0) {
