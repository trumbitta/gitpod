...
function AllTeams() {
  ...
  const [isChargebeeCustomer, setIsChargebeeCustomer] =
  ...
  const [createTeamModal, setCreateTeamModal] = useState<
    { types: string[]; defaultCurrency: string } | undefined
  >(undefined);
  const [manageTeamModal, setManageTeamModal] = useState<
    { sub: TeamSubscription } | undefined
  >(undefined);
  const [inviteMembersModal, setInviteMembersModal] = useState<
    { sub: TeamSubscription } | undefined
  >(undefined);
  ...
  const restorePendingPlanPurchase = () => {
    const pendingState = restorePendingState('pendingPlanPurchase') as
      | { planId: string }
      | undefined;
    return pendingState;
  };
  ...
  const restorePendingSlotsPurchase = () => {
    const pendingState = restorePendingState('pendingSlotsPurchase') as
      | { tsId: string }
      | undefined;
    return pendingState;
  };
  ...
  const [pendingSlotsPurchase, setPendingSlotsPurchase] = useState<
  ...
  const queryState = async () => {
    try {
      const [slots, showPaymentUI, teamSubscriptions, clientRegion, isStudent] =
        await Promise.all([
      ...
      setDefaultCurrency(
        clientRegion && (countries as any)[clientRegion]?.currency === 'EUR'
          ? 'EUR'
          : 'USD'
      );
  ...
  useEffect(() => {
      ...
      (async () => {
        try {
          setIsChargebeeCustomer(
            await getGitpodService().server.isChargebeeCustomer()
          );
        } catch (error) {
          console.log(error);
        }
    ...
    if (pendingPlanPurchase) {
      if (
        teamSubscriptions.some((ts) => ts.planId === pendingPlanPurchase.planId)
      ) {
  ...
  useEffect(() => {
    if (pendingSlotsPurchase) {
      if (
        slots.some((s) => s.teamSubscription.id === pendingSlotsPurchase.tsId)
      ) {
  ...
  const getSubscriptionTypes = () =>
    isStudent
      ? ['professional', 'professional-new', 'student']
      : ['professional', 'professional-new'];

  const getActiveSubs = () => {
    const now = new Date().toISOString();
    const activeSubs = (teamSubscriptions || []).filter((ts) =>
      TeamSubscription.isActive(ts, now)
    );
    return activeSubs;
  };

  const getAvailableSubTypes = () => {
    const usedTypes = getActiveSubs().map(
      (sub) => getPlan(sub)?.type as string
    );
  ...
  const slotDoPoll = (slot: TeamSubscriptionSlotResolved) => () =>
    pollForSlotUpdate(slot);
    ...
    poll<TeamSubscriptionSlotResolved>(
      ...
      async () => {
        ...
        return {
            ...
            freshSlot.state !== slot.state ||
            freshSlot.assigneeId !== slot.assigneeId,
          result: freshSlot,
        };
  ...
  const updateSlot = (
    slot: TeamSubscriptionSlotResolved,
    update: (s: Slot) => Slot
  ) => {
    setSlots((prevState) => {
  ...
  const slotSetErrorMessage =
    (slot: TeamSubscriptionSlotResolved) => (err: any) => {
      updateSlot(slot, (s) => {
        const result = { ...s };
        result.errorMsg =
          err && ((err.data && err.data.msg) || err.message || String(err));
  ...
  const slotInputHandler = {
    ...
    reassign: (
      ...
      updateSlot(slot, (s) => ({ ...s, loading: true }));
      getGitpodService()
        .server.tsReassignSlot(
          slot.teamSubscription.id,
          slot.id,
          newAssigneeIdentifier
        )
        .then(slotDoPoll(slot))
        .catch(slotSetErrorMessage(slot));
    },
      ...
      async () => {
        ...
        if (
          teamSubscriptions.some(
            (t) =>
              TeamSubscription.isActive(t, now) && t.planId === plan.chargebeeId
          )
        ) {
  ...
  const getPlan = (sub: TeamSubscription) => {
    return Plans.getAvailableTeamPlans().filter(
      (p) => p.chargebeeId === sub.planId
    )[0];
  };
    ...
    <React.Fragment>
      <div className="flex flex-row">
        ...
        <div className="flex flex-end space-x-3">
          {isChargebeeCustomer && (
            <button
              className="self-end my-auto secondary"
              onClick={() => showBilling()}
            >
              Billing
            </button>
          ...
          {getActiveSubs().length > 0 && (
            <button
              ...
              disabled={
                !!pendingPlanPurchase || getAvailableSubTypes().length === 0
              }
      ...
      {createTeamModal && (
        <NewTeamModal
          onClose={() => setCreateTeamModal(undefined)}
          onBuy={onBuy}
          {...createTeamModal}
        />
      )}
      ...
      {inviteMembersModal && (
        <InviteMembersModal
          onClose={() => setInviteMembersModal(undefined)}
          {...inviteMembersModal}
        />
      )}
      ...
      {addMembersModal && (
        <AddMembersModal
          onClose={() => setAddMembersModal(undefined)}
          onBuy={onBuy}
          {...addMembersModal}
        />
      )}
          ...
          {pendingPlanPurchase && (
            ...
            >
              <div className="px-4 self-center w-1/12">
                <div
                  className={
                    'rounded-full w-3 h-3 text-sm align-middle bg-gitpod-kumquat'
                  }
                >
                  &nbsp;
                </div>
      ...
      <div className="border-t border-b border-gray-200 dark:border-gray-800 mt-2 -mx-6 px-6 py-4 space-y-2">
        ...
        <div className="flex flex-col space-y-2">
          ...
          <p className="pb-4 text-gray-500 text-sm">
            {copied
              ? 'Copied to clipboard!'
              : 'Use this URL to join this team.'}
          </p>
        </div>
      </div>
...
function AddMembersModal(props: {
  ...
  const getPlan = () => {
    return Plans.getAvailableTeamPlans().filter(
      (p) => p.chargebeeId === props.sub.planId
    )[0];
  };
  ...
  return (
    <Modal visible={true} onClose={props.onClose}>
      ...
      <div className="flex justify-end mt-6">
        <button
          className={'ml-2'}
          onClick={() => props.onBuy(getPlan(), quantity, props.sub)}
        >
          Continue to Billing
        </button>
      </div>
...
function NewTeamModal(props: {
  ...
  const getPlan = (type: string, currency: string) => {
    return Plans.getAvailableTeamPlans().filter(
      (p) => p.type === type && p.currency === currency
    )[0];
  };
  ...
  const [plan, setPlan] = useState<Plan>(
    getPlan(props.types[0], props.defaultCurrency)
  );
  const [quantity, setQuantity] = useState<number>(5);
          ...
          {slots.map((slot, index) => {
            return (
              <SlotInput
                key={slot.id}
                slot={slot}
                inputHandler={props.slotInputHandler}
              />
            );
          })}
        </div>
...
function SlotInput(props: { slot: Slot; inputHandler: SlotInputHandler }) {
  ...
  useEffect(() => {
    setEditMode((prev) => {
      const newEditMode =
        (prev && !!props.slot.loading) || !!props.slot.errorMsg;
      return newEditMode;
    });
  ...
  return (
    <div key={key} className="flex flex-col space-y-2 pt-2">
      ...
      <div className="flex flex-grow flex-row space-x-2">
        <input
          ...
          value={
            editMode
              ? assigneeIdentifier || ''
              : renderIdentifier(slot.assigneeIdentifier) || ''
          }
      ...
      {slot.state === 'deactivated' && (
        <p className="pb-4 text-gray-500 text-sm">
          You will no longer be billed for this seat starting{' '}
          {formatDate(slot.cancellationDate)}.
        </p>
      )}
      {errorMsg && (
