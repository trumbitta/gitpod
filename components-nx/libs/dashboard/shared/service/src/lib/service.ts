...
function createGitpodService<C extends GitpodClient, S extends GitpodServer>() {
  if (window.top !== window.self && process.env.NODE_ENV === 'production') {
    const connection = createWindowMessageConnection(
      'gitpodServer',
      window.parent,
      '*'
    );
...
export function getGitpodService(): GitpodService {
  ...
  if (window.location.search.includes('service=mock')) {
    const service =
      _gp.gitpodService || (_gp.gitpodService = gitpodServiceMock);
    return service;
  }
  const service =
    _gp.gitpodService || (_gp.gitpodService = createGitpodService());
  return service;
}
