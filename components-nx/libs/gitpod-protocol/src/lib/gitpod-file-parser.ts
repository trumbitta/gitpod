...
import * as schema from '../assets/json/gitpod-schema.json';
  ...
  public parse(
    ...
    try {
      ...
      const validationErrors = validate.errors
        ? validate.errors.map((e) => e.message || e.keyword)
        : undefined;
      ...
      if (!options.acceptPortRanges && Array.isArray(parsedConfig.ports)) {
        overrides.ports = parsedConfig.ports.filter(
          (port: any) => !PortRangeConfig.is(port)
        );
      }
      return {
        config: {
      ...
      return {
        config: defaultConfig,
        validationErrors: [
          'Unparsable Gitpod configuration: ' + (err as Error).toString(),
        ],
      };
    }
  }
